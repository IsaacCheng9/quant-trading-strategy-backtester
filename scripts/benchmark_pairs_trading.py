...
combinations. This isolates the pandas computation from network I/O.
"""
...
NUM_RUNS = 6
OUTPUT_CSV = "resources/pandas_pairs_trading_benchmark.csv"


def download_pair_data(ticker1: str, ticker2: str) -> pd.DataFrame | None:
    """
    Download and format price data for a ticker pair.

    Returns:
        A pandas DataFrame with Close_1 and Close_2 columns
        (Date as index), or None if the download fails.
    """
    try:
...
def main():
    ...
    pair_data: dict[tuple[str, str], pd.DataFrame] = {}
